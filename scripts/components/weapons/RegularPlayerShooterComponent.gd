...
func shoot(delta: float) -> void:
	if Input.is_action_pressed("shoot") and fire_cooldown_frames == 0:
		fire_cooldown_frames = (1000.0 / shooter_owner.firerate) * delta
		for bs in range(min(shooter_owner.bullets_on_shot, 2 * PI / shooter_owner.spread)):
			...
			bullet.speed = shooter_owner.bullet_speed
			var rot_mod: float = 0
			for it_add in range(0, bs + 1):
				rot_mod += it_add * pow(-1, it_add)
			if shooter_owner.bullets_on_shot % 2 == 0:
				bullet.rotation += pow(-1, shooter_owner.bullets_on_shot) * shooter_owner.spread / 2
			bullet.rotation += rot_mod * shooter_owner.spread
